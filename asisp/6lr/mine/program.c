// 10. На потоках. Найти  эффективный размер кластера на диске,
// подсчитав в % отношении общий объем занимаемый файлами в
// заданном каталоге и ниже  к реальному размеру этих файлов.
// Для каждого подкаталога запускается отдельный процесс подсчета.
// Количество процессов  в любой момент времени должно быть равно N.
// Данные передаются с использованием общего файла.
// Использовать семафор  для доступа к общему файлу.
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <string.h>
...
void *thread_func(void *arg)
{
    /* shared local variable, one instance among all threads using this function, * so a semaphore is needed to enforce mutual exclusion.  Because the * semaphore must be initialized, only once, the initialization takes place * in the main thread, and the semaphore is declared globally.  You should * be able to explain why the semaphore cannot safely be declared inside * thread_func() and initialized from within the thread function itself. * This problem could be corrected with a C++ constructor for a semaphore * class. */ static int count = 0; /* static sem_t count_sem; */ 
...
void Sem_getvalue(sem_t *sem, int *sval, char *msg)
{
    int ret;
    if (sem_debug)
        printf("sem_getvalue: thread %d, %s\n", pthread_self(), msg);
