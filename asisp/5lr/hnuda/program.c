// 7.Написать программу подсчета частоты встречающихся символов
// в файлах заданного каталога его подкаталогов.
// Пользователь задаёт имя каталога.
// Главный процесс открывает каталоги и запускает для каждого
// файла каталога отдельный поток подсчета частоты встречающихся символов.
// Каждый поток выводит на экран свой id, полный путь к файлу, общее число
// просмотренных байт, символы и их частоты (все в одной строке через пробелы).
